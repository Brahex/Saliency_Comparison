...
class GuidedBackpropReLUFunction(torch.autograd.Function):
    ...
    def __init__(self, inplace=False):
        super(GuidedBackpropReLUFunction, self).__init__()
...
class GuidedBackpropReLU(nn.Module):
    ...
    def forward(self, x):
        output = GuidedBackpropReLUFunction.apply(x)
        return output
'''    
    def backward(self, x):
        output = GuidedBackpropReLUFunction.apply(x)
...
# Own Addition
class BottleneckGBR(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1):
        super(BottleneckGBR, self).__init__()
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = FixedBatchNorm(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=dilation, bias=False, dilation=dilation)
...
# Own Addition    
class ResNetGBR(nn.Module):

    def __init__(self, block, layers, strides=(2, 2, 2, 2), dilations=(1, 1, 1, 1)):
        self.inplanes = 64
        super(ResNetGBR, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=strides[0], padding=3,
                               bias=False)
        self.bn1 = FixedBatchNorm(64)
        self.relu = GuidedBackpropReLU()
...
# Own Addition
def resnet50GBR(pretrained=True, **kwargs):

    model = ResNetGBR(BottleneckGBR, [3, 4, 6, 3], **kwargs)
    if pretrained:
        state_dict = model_zoo.load_url(model_urls['resnet50'])
        state_dict.pop('fc.weight')
        state_dict.pop('fc.bias')
